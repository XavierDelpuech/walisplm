...
# Traduction en langage R : Xavier Delpuech, IFV, Domaine de Donadille, 30230 Rodilhan. xavier.delpuech@vignevin.com , f?vrier 2012
# Bibliographie
                                    ...
                                    az_r=0, poro_r=0.25, alb_f=0.22, alb_s=0.18, Dt=1800, r_rgi=20)
{
  ...
  #r_lat = latitude    # =44.5
  #r_longit = 0
  #az_r = azimut # =0   Azimut en degr?s (ex: 0=NS ; 90=EW ; -45 = NW-SE )
  #d_r = d      # =1.5 Distance inter-rang (en m?tres)
  #alb_f = 0.22
  #alb_s = alb_sol  # =0.18
  #i_year = n_an
  ...
  #poro_r = poro   # =0.25 Porosite min en ete, de 0 ? 1 (0=aucun trou dans le feuillage)
  ...
  rLongitude = r_longit
  iYear = i_year
  rOrientRangs = az_r     #? l#entr?e :   0 pour rangs NS, 90 pour EW et -45 pour NW-SE
  #ici en entr?e, rOrientRangs  en degr?s :    ( 0=NS ; 90=EW )  ( -45 = NW-SE )
  #passage direction des rangs ? angle de la normale aux rangs avec la direction NS
  rOrientRangs = rOrientRangs + 90                            #orient?e de N ? S
  ...
  #orientation comme angles d#azimut solaire, ? partir du Sud, n?gatifs le matin et positifs l#apr?s-midi
  rEcartRangs = d_r
  rHautRang = hf_r
  rLargRang = lf_r
  rPorosite = poro_r
  ...
  #variables d#entree :
  iSimulDay = i_doy
  rRgiJour = r_rgi * 1000000               #MJ.m^-2  ?  J.m^-2

  # *****************************************  DEBUT  JOURNEE  ***************************************

  rSJRytGlo = 0           #intialisation des int?grales journali?res Rayonnement
  rSJRytDif = 0
  rSJRytGloVigne = 0
  rSJRytGloSol = 0
  rSJAlbedoVignoble = 0
  ...
  #    Call ParSolJour((iYear), (iSimulDay)) # remplac? par le sous-programme lui-m?me
  ...
  #   ATTENTION
  #        Ce SOUS-PROGRAMME ne peut etre execut? qu'apres le SOUS-PRO-
  ...
  # Call ParSolJourBis((iYear), (iSimulDay)) # remplac? par le sous-programme lui-m?me
  ...
  #
  #   Pour passer ? l'heure legale, ajouter le d?calage par rapport a l'heure TU
  ...
  #        Il faut aussi preciser le lieu en d?finissant
  #
  ...
  #Dur?e du jour, avec rRefrac, Correction de r?fraction atmosph?rique
  rRefrac = -0.0106
  ...
  if (rHautRang>0) {  #********************  DEBUT BOUCLE rHautRang > 0  ***********************************
    ...
    rOrientRangsRad = rOrientRangs * pi / 180    #O? rOrientRangs = ANGLE NORMALE AU RANG AVEC DIRECTION NORD-->SUD (DEGRES)

    #D?but Appel Sous-Programmes Rayonnement
    #       Sub AbsDiffus()
    ...
    #  c    porosit? du feuillage rPorosite
    ...
    # ... fraction du rayonnement diffus intercept? par la masse foliaire de la vigne
    #par les parties sup?rieures des rangs (horizontales et non poreuses)
    ...
    #facteur de forme entre les parois horizontales entre les rangs, sup?rieure et inf?rieure.
    rFFdif = tan(0.5 * atan((rEcartRangs - rLargRang) / rHautRang))
    #par les parois verticales (d'abord suppos?es non poreuses)                 'eq 16 de Riou et al., 1989
    rKDIF2 = (1 - rFFdif)
    rKDIF2 = rKDIF2 * (1 - rLargRang / rEcartRangs)
    #par les parties inf?rieures des rangs (horizontales et non poreuses)       'eq 20 de Riou et al., 1989
    ...
    #       calcul des taux d'absorption (cardif) et de r?flexion (crrdif) du rayt.diffus
    ...
    #            Sub PosGeomLim()
    #       Positions G?om?triques Limites
    ...
    #(le temps rTempsTUFirst lu dans data_H doit ?tre en heures TU decimales et centr?es)
    j = 0                                                        #calculs ? partir de 0 h
    ...
    repeat
    {
      ...
      # sous-programme        ParSolHor((rHr))                #param?tre rHr = Heure TU (h)
      ### ParSolHor
      #      Calcul des rHautSol et rAzimSol du Soleil (degres) ? l#heure rHr (en h TU)
      #      du jour iNJ de l#annee iAn pour un lieu de Latitude rLatitude, de
      #      Longitude rLongitude (degres), d#?quation du temps rEquaTemps (minutes),
      #      et de d?clinaison du Soleil rDeltaRad (radians)
      #
      #      ATTENTION
      #        Avant l#utilisation de ce SOUS-PROGRAMME, il faut avoir execut?
      #      les SOUS-PROGRAMMES ParSolAnn(iAn%) et ParSolJour(iNJ%, iAn%)
      #        Il faut avoir precis? le lieu par sa rLatitude et sa rLongitude (degres)
      #
      rLatRad = rLatitude * pi / 180
      ...
      if (X < 0) rHautSol = 0  #-99.9                    #le Soleil n#est pas lev?
      rHautSol=ifelse ((X==1), 90, asin(X) * 180 / pi)          # XD : v?rifier les radians
      ...
      rAHDeg = (rHeureTSV - 12) * 15                        #angle horaire  (degr?s)
      ...
      #Microclimat_Feuilles                         #MsgBox "Microclimat_Feuilles  Termin?"

      #rSJTleaf = rSJTleaf + rTleaf                #mise ? jour des int?grales journali?res microclimat
      #rSJDefSat = rSJDefSat + rDefSat

      if (rHautSol>0) {        #DEBUT TEST rHautSol > 0  pour partie Ryt Solaire
        #rSJTleafDiurne = rSJTleafDiurne + rTleaf    #mise ? jour des int?grales journ. microclimat periode diurne
        ...
        if (rRytGloExtraAtm>0) {             #D?but calcul Rayonnement Diffus
          GSG0 = rRytGlo / rRytGloExtraAtm
        }else{GSG0 = 0
        }


        #DSG = 1.09 - 2.6896 * GSG0 * GSG0 + 1.2843 * GSG0 * GSG0 * GSG0   # relation journali?re ? Bordeaux (CV)
        DSG = 1.09 - 2.44 * GSG0 * GSG0 + 1.084 * GSG0 * GSG0 * GSG0   # relation horaire ? Bordeaux (CV)
        ...
        #       calcul des taux d'absorption (cardir) et de de r?flexion (crrdir) du rayt. direct
      ...
      rSJRytGlo = rSJRytGlo + rRytGlo * rDt                 #Mise ? Jour des Int?grales Journali?res / Rayonnement
    ...
    # ------------------------------  mise ? jour des int?grales journali?res (rSJ...)  ---------------------
  ...
  # ------------------------------  sorties journali?res  ------------------------------------
  #                           (int?grales sur la dur?e de la journ?e)
  ...
  if (rHautRang>0){   #donc si realisation calculs int ryt (et event. photos.) dans boucle horaire
    ...
    sor_ryt[10,1] = az_r     #comme en entr?e :   0 pour rangs NS, 90 pour EW et -45 pour NW-SE
    ...
    sor_ryt[24,1] = iSJdiurne #nb d#intervalles utilis?s pour calculer les integrales SJ (jour entier)
    sor_ryt[25,1] = iSJtot     #nb d#intervalles utilis?s pour calculer les integrales SJ (per. diurne)
  }else{
    for (i in 9:25) {
      sor_ryt[i,1] = 0
    }
